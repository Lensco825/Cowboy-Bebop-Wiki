...
exampleImages.forEach(example => {
    ...
    const yPosition = elementStyle.getPropertyValue("bottom");     
    heroSection.addEventListener('mousemove', (e) => {
       let mouseX = e.pageX;
        let mouseY = e.pageY;
        let offSetX = example.getBoundingClientRect().width / 2;
        let offSetY = example.getBoundingClientRect().height / 2;
        example.style.left = `calc(${xPosition} - ${mouseX - offSetX}px * 0.1)`;
        example.style.top = `calc(${yPosition} - ${mouseY - offSetY}px * 0.1)`;
